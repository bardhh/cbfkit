...
def cbf(obstacle: float, obstacle_radius: float, **kwargs) -> Callable[[Array], Array]:
    ...
    def func(state_and_time: Array) -> Array:
        ...
        """
        x = state_and_time
        return (x[0] - obstacle[0]) ** 2 + (x[1] - obstacle[1]) ** 2 - obstacle_radius**2

    return func


def cbf_grad(obstacle: float, obstacle_radius: float, **kwargs) -> Callable[[Array], Array]:
    ...
    """
    jacobian = jacfwd(cbf(obstacle, obstacle_radius, **kwargs))
...
def cbf_hess(obstacle: float, obstacle_radius: float, **kwargs) -> Callable[[Array], Array]:
    ...
    """
    hessian = jacrev(jacfwd(cbf(obstacle, obstacle_radius, **kwargs)))
