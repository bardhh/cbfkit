...
def search_sorted(time_stamps, time, start_lower_index):
    def search_body(i, bounds):
        ...
        condition = time_stamps[start_lower_index + middle] < time
    ...
    lower, upper, middle = 0, time_stamps.shape[0] - start_lower_index - 1, 0
    ...
    return start_lower_index + middle
...
def jax_global(lower_time_bound, upper_time_bound, robustness, time_stamps):
    ...
    def bounded_case(_):
        ...
        return vmap(compute_global)(jnp.arange(length))
...
# Processes mtl until
@jit
def jax_until(lower_time_bound, upper_time_bound, left_robustness, right_robustness, time_stamps):
...
if __name__ == "__main__":
    ...
    time_stamps = jnp.linspace(0, 10, length)
