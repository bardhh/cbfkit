...
def nimble_ant_c(x, params):
    ...
    for idxi, _ in enumerate(bad_sets):
        ...
        g1 = (
            -1 * (2 * math.sin(x[2])) * (x[0] - curr_bs[0]) / curr_bs[2] ** 2
            - 2 * math.cos(x[2]) * (x[1] - curr_bs[1]) / curr_bs[3] ** 2
        )
        g2 = (
            4 * ((x[0] - curr_bs[0]) / curr_bs[2]) ** 2
            + 4 * ((x[1] - curr_bs[1]) / curr_bs[3]) ** 2
            + 4 * math.cos(x[2]) * (2 * x[0] - 2 * curr_bs[0]) / curr_bs[2] ** 2
            + 4 * (math.sin(x[2])) * (2 * x[0] - 2 * curr_bs[1]) / curr_bs[3] ** 2
        )
...
for idxi, i in enumerate(xx):
    for idxk, k in enumerate(yy):
        ...
        t, y, x = control.input_output_response(
            sys=silly_bug_sys, T=T, U=0, X0=[i, k, 0], return_x=True, method="BDF"
        )
