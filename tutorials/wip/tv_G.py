...
from sympy import Matrix, exp, sqrt, symbols
...
dt = T[1] - T[0]
params = {"x_goal": x_goal, "bad_sets": bad_sets, "ctrl_param": ctrl_param, "CBF": my_CBF}
...
(line1,) = ax1.plot([], [], lw=2)
goal_square = plt.Rectangle(x_goal - np.array([1, 1]), 1, 1, color="g", alpha=0.5)
...
dist_text.set_text(
    "dist(||x - [10 5]^T||< 10, t = 5) = " + "{:.2f}".format(dist_to_goal[find_nearest(T, 5)])
)

dist_text_3 = ax2.text(
    0.2, 1.10, "G_[5,15](||x - [10 5]^T||< 10)", transform=ax2.transAxes, fontsize=12
)

# plot patch of halfspace x bellow 0
patch_x_below_0 = plt.Rectangle((0, pred), 20, -20, color="r", alpha=0.2)
