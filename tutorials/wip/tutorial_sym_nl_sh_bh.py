...
from sympy import Matrix, Symbol, cos, sin, sqrt, symbols
...
def n_omni_robots_f(t, x, u, params):
    ...
    for i in range(0, x.shape[0]):
        # np.append(g,[])
        newg[i] = np.array(
            [[[cos(x[i][2]), -1 * sin(x[i][2]), 0], [sin(x[i][2]), cos(x[i][2]), 0], [0, 0, 1]]]
        )
...
def example(i):
    ...
    switcher = {
        0: [[3, 2.0, 1.0, 1.0]],
        1: [[1.0, 2.0, 0.5, 0.5], [4.0, 1.0, 0.5, 0.5], [3.0, 2.0, 0.5, 0.5], [4.5, 4.2, 0.5, 0.5]],
...
xr0, xr1, xr2, u = symbols("xr0 xr1 xr2 u")
...
Pg = Matrix([[cos(xr2), -1 * sin(xr2), 0], [sin(xr2), cos(xr2), 0], [0, 0, 1.0]])
g = Pg * Bgg
