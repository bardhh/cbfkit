...
import tutorials.multi_robot_di as multi_robot_di
...
for i in range(len(state_constraint_funcs)):
    module_name = f"multi_robot_di.certificate_functions.barrier_functions.barrier_{i+1}"
    ...
    cbf_package = rectify_relative_degree(
        function=h,
        system_dynamics=multi_robot_di.plant(),
        state_dim=STATE_DIM,
        form="exponential",
...
nominal_controller = multi_robot_di.controllers.controller_1(goal=goals, k_p=KP, k_d=KD)
...
mppi_local_planner = mppi_planner.vanilla_mppi(
    ...
    dynamics_func=multi_robot_di.plant(),
