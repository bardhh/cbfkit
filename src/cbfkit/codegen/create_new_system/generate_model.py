...
def run_black(file_paths: List[str]) -> None:
    """Format the given files with Black in a single subprocess call, if Black is available.

    Black ships in the optional ``codegen`` extra (``pip install cbfkit[codegen]``).
    When it is not installed the files are left unformatted (still valid Python) and
    a warning is logged, so code generation works on a slim ``pip install cbfkit``.

    All files generated by a single ``generate_model`` call are batched into one
    ``black`` invocation (rather than spawning a subprocess per file) since each
    subprocess spawn costs tens of milliseconds of interpreter startup.
    """
    if not file_paths:
        return
    try:
        ...
        LOGGER.warning(
            "Black is not installed; leaving %d generated file(s) unformatted. Install "
            "the codegen extra for auto-formatted output: pip install cbfkit[codegen]",
            len(file_paths),
        )
    except subprocess.CalledProcessError as exc:
        stderr = exc.stderr.decode().strip() if exc.stderr else str(exc)
        LOGGER.error("Black failed to format generated files: %s", stderr)
        raise RuntimeError(f"Black failed to format generated files: {stderr}") from exc
...
def generate_model(
    ...
    # Paths of every file generated below, formatted with Black in one batched
    # subprocess call at the end instead of one spawn per file.
    ...
    generated_files.append(model_folder + DELIMITER + "__init__.py")
    ...
    generated_files.append(model_folder + DELIMITER + "plant.py")
    ...
    if stage_cost_function is not None:
        ...
        generated_files.append(stage_cost_folder + DELIMITER + "stage_cost.py")
    ...
    if terminal_cost_function is not None:
        ...
        generated_files.append(terminal_cost_folder + DELIMITER + "terminal_cost.py")
    ...
    if barrier_funcs is not None:
        ...
        for bb in range(n_bars):
            ...
            generated_files.append(barrier_folder + DELIMITER + f"barrier_{bb+1}.py")
    ...
    if lyapunov_funcs is not None:
        ...
        for ll in range(n_lfs):
            ...
            generated_files.append(lyapunov_folder + DELIMITER + f"lyapunov_{ll+1}.py")
    ...
    if nominal_controller is not None:
        ...
        for cc in range(n_cons):
            ...
            generated_files.append(control_folder + DELIMITER + f"controller_{cc+1}.py")
    ...
    # Format every generated file with a single batched Black subprocess call,
    # instead of spawning one subprocess per file above.
    run_black(generated_files)
