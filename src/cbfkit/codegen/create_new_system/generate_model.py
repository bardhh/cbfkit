...
def generate_model(
    ...
    from controllers import (
        controller_1,
    )  #USER_DEFINED. This should be user defined. Current text works with the controller generated by van_der_pol oscilator tutorial. Import the appropriate controller for your application.
    ...
    class VanDerPolOscillatorController(Node):
        ...
        def get_controller_by_name(name, **kwargs):
            ...
            module = __import__('controllers', fromlist=[name])
    ...
    class VanDerPolOscillatorSensor(Node):
        ...
        def __init__(self):
            super().__init__("van_der_pol_oscillator_sensor")
    ...
    class VanDerPolOscillatorEstimator(Node):
        ...
        def __init__(self):
            super().__init__("van_der_pol_oscillator_estimator")
    ...
    class VanDerPolOscillatorPlant(Node):
        ...
        def __init__(self):
            super().__init__("van_der_pol_oscillator_plant")
        ...
        def get_dynamics_by_name(name, **kwargs):
            ...
            '''
            dynamics_module = __import__(name, fromlist=[name])
    ...
    # Defaults for van_der_pol_oscilator
    ...
    for script in node_scripts[:-1]:  # Exclude the last script for now
        bash_script_contents += f"python3 {script} &\n"

    # Add the last script without the '&' to avoid running it in the background
    bash_script_contents += f"python3 {node_scripts[-1]}"
