...
def initialize_mppi_plots(ax, plot_num_samples, prediction_horizon, robot_state_dim):
    ...
    for i in range(plot_num_samples):  # sampled trajectories
        sampled_trajectories_plot.append(
            ax.plot(jnp.ones(prediction_horizon), 0 * jnp.ones(prediction_horizon), "g", alpha=0.2)
        )
    selected_trajectory_plot = ax.plot(
        jnp.ones(prediction_horizon), 0 * jnp.ones(prediction_horizon), "b"
    )
    return {
...
def update_mppi_plot(mppi_plots, robot_sampled_states, robot_selected_states):
    for i in range(mppi_plots["plot_num_samples"]):
        mppi_plots["sampled_trajectories_handle"][i][0].set_xdata(
            robot_sampled_states[mppi_plots["robot_state_dim"] * i, :]
        )
        mppi_plots["sampled_trajectories_handle"][i][0].set_ydata(
            robot_sampled_states[mppi_plots["robot_state_dim"] * i + 1, :]
        )
