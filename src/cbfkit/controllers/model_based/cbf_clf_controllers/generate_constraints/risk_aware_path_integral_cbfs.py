...
def generate_compute_ra_pi_cbf_constraints(
    ...
    **kwargs: Dict[str, Any],
    ...
    ra_params.ra_cbf.integrator_states = jnp.zeros((n_bfs,))
    ...
    def compute_cbf_constraints(t: float, x: State) -> Tuple[Array, Array]:
        ...
        ra_params.ra_cbf.integrator_states = lax.cond(
            t == 0, lambda _: jnp.zeros((n_bfs,)), lambda _: ra_params.ra_cbf.integrator_states, 0
        )

        if n_bfs > 0:
            bf_x, bj_x, bh_x, dbf_t, _ = compute_barrier_values(t, x)
            w_vals = ra_params.ra_cbf.integrator_states + ra_params.ra_cbf.gamma + r_buffer
