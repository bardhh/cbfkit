...
import cbfkit.simulation.status as status_mod


def _patch_numpy_reductions(monkeypatch):
    monkeypatch.setattr(status_mod.jnp, "any", lambda arr: bool(np.any(arr)))
    monkeypatch.setattr(status_mod.jnp, "argmax", lambda arr: np.argmax(arr))
    monkeypatch.setattr(status_mod.jnp, "sum", lambda arr: np.sum(arr))
...
def test_check_simulation_status_treats_status_2_as_failure_warning(monkeypatch):
    ...
    monkeypatch.setattr(status_mod, "print_warning", lambda message: warnings.append(message))
    monkeypatch.setattr(status_mod, "print_error", lambda _message: None)
