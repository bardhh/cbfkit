...
bad_sets = [] #tut_cbf_utils.example(1)
...
for idxj, j in enumerate(bad_sets):
    ...
    assert tut_cbf_utils.is_inside_ellipse(
...
for i in range(len(T)-1):
    print(T[i])
    x[:, i+1] = x[:, i] + dt * \
        np.array(tut_sys_and_ctrl.nimble_ant_tv_f(T[i], x[:, i], [], params))
...
# Animate
ax1 = tut_cbf_utils.plot_cbf_elements_circle(ax1, bad_sets, x_goal_2, 5, 'g')
ax1 = tut_cbf_utils.plot_cbf_elements_circle(ax1, bad_sets, x_goal_1, 10, 'r')
