...
from ..cbflib import tut_scbf, tut_sys_and_ctrl
...
for i in range(len(T) - 1):
    xr_sim[:, i + 1] = (
        ...
        * (
            np.array(
                tut_sys_and_ctrl.approx_unicycle_agent_f(T[i], xr_sim[:, i], [], params)
            )
        ).T
    )
    xo_sim[:, i + 1] = (
        xo_sim[:, i]
        + dt
        * (np.array(tut_sys_and_ctrl.simple_agent_left_f(T[i], xo_sim[:, i], [], params))).T
    )
